...
Full JSON schema for WurmTweaker creature definitions.
...
- Changes are applied on server start.
- The `spawns` array is **ignored** for vanilla creatures — spawn entries only apply to new custom creatures.
...
- **≥ 120** — new custom creature. IDs above 119 are safe for custom use.
...
**Horse and colour variant models:** when `isHorse: true` or type flag `64` is set, the client model path is assembled as:
...
The variant model files must already exist in the client — vanilla models or added via serverpacks.

**Standard horse colour names** (13 entries):
...
Determines body slot layout, hitbox shape, and which equipment slots are available. `0` is the standard humanoid layout. Other values correspond to animal and dragon body forms — only change this if you have a confirmed value for the body type you need.
...
- Goblin: `100`
- Troll: `300`
- Omitting leaves the vanilla default unchanged.
...
Array of type flag IDs. Multiple IDs stack; order does not matter. Type `31` is reserved — do not use it.

→ Full table: `data/creatures/references/creatureTypes.md`
...
Primary combat skill rating. Governs attack and defence rolls against players and other creatures.
...
Primary wound type applied to the creature's normal melee attacks. Governs armor wear calculations and the wound label shown in the combat log.
...
| `7` | `TYPE_WATER` | Water | Drowning damage — no vanilla creature uses this as their primary damage type |
...
**Armor wear:** types `0`–`4` (physical types) can cause 2× or 4× armor wear on hit depending on the armor type. Types `5`–`8` do not increase armor wear — armor degrades more slowly when struck by poison, infection, water, or cold attacks.

**Breath attacks are independent of this field.** Breath combat moves set their own wound type, overriding this field. Setting `damageType: 4` (burn) on a dragon only affects its normal melee swings — the fire breath move always deals burn regardless, and cold breath always deals cold.
...
Each move has a per-tick trigger probability (`rarity`) — on any given combat tick the server rolls against this chance for each move in the creature's pool.
...
- **`HELLHORSEFIRE` (10)** uses the same fire cone implementation as `FIREBREATH` (3) but is registered separately at much lower damage and rarity. Give it only to hellhorse-type creatures.
...
Armor tier used in combat damage calculations. Both the bare constant name and the `ArmourTemplate.` prefixed form are accepted.
...
Defines the creature's attack system. Two systems (legacy and new) can coexist in the definition — only one is active at runtime depending on `useNewSystem`.

#### `combat.attacks.useNewSystem` · Boolean · Optional (default: `false`)

When `true`, the engine uses the `primary` and `secondary` attack lists at runtime, ignoring legacy damage values. The legacy block should still be populated even when this is `true` — the values serve as a fallback reference.

---

#### `combat.attacks.legacy` · Object · Optional

The classic single-value attack system. Always stored regardless of `useNewSystem`. Only active at runtime when `useNewSystem` is `false`.
...
#### `combat.attacks.primary` / `combat.attacks.secondary` · Array · Optional
...
| `identifier` | String | Attack type constant. See valid values below. |
| `values` | Object | Attack parameters — see below |
...
An unrecognized identifier logs a warning and skips that attack entry without crashing the server.
...
Top-level fallback for `combat.combatMoves`. Ignored if `combat.combatMoves` is also set. See the `combat.combatMoves` section above for the full move table.
...
→ Full skill ID table: `data/skills/references/skills.md`
...
Omitting a skill leaves it at the vanilla default. For editing vanilla creatures: only the keys you include are overridden.
...
Collision bounding box in local space: `[minX, minY, maxX, maxY]`. Must contain exactly 4 values.
...
Each string is a colour segment inserted into the client model path. The engine strips spaces and lowercases the colour name before assembling the final path as `[modelName].[colourName].[sex]` — for example, `model.creature.quadraped.horse.grey.male` or `model.creature.quadraped.horse.piebaldpinto.female`. The variant model files must exist in the client — you cannot invent new names unless the corresponding client assets exist.

**This system is only active when:**
- `isHorse: true`, **or**
- type flag `64` (Multi-color / horse colors) is in the `types` array

For all other creatures, the first entry in the array is always used and the rest have no visible effect.

**How colour selection works:** each creature instance carries a set of trait bits. Specific trait bits map to an array index, which selects the colour name. The array position is fixed — you cannot reorder the meaning of positions.
...
The array is populated from position 0 upward (maximum 13 entries). A creature whose trait resolves to an index beyond the array length falls back to index 0.

Hell horses use only 8 entries — trait bits 30–34 would map to indices 8–12, which exceed the array length and fall back to index 0 (`ash`).
...
Base RGB tint applied to the model. `{255, 255, 255}` = no tint (pure model colors).
...
Registers this creature into Wurm's encounter system. Each entry adds the creature to one tile-type + elevation spawn pool. Ignored for vanilla creature overrides.

| Field | Type | Default | Notes |
|---|---|---|---|
| `tile` | String | — | Tile type name. See `data/creatures/references/spawnTiles.md` |
...
Placeholder field for specifying a custom AI behaviour class. Reserved for future use — no behaviour is currently applied.

### `note` · String · Optional

Free-text annotation. Not used by the mod. Exists purely for documentation inside the JSON file.
